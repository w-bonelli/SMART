...
from os.path import join, abspath, splitext, getsize, basename, dirname
...
from skimage import img_as_ubyte, img_as_float, img_as_bool, morphology
...
def comp_external_contour_1(orig, thresh, output_directory):
    ...
    for c in contours:
        ...
        if w > 0 and h > 0:
            ...
            cv2.imwrite(join(output_directory, str(format(index, "02")) + '.png'), roi)
...
def segmentation(image_file, output_directory):
    file_name, _ = splitext(image_file)
    file_size = getsize(image_file) / float(1 << 20)

    print("Segmenting image {0} \n".format(str(file_name)))
    if file_size > 5.0: print("It will take some time due to large file size {0} MB".format(str(int(file_size))))

    image = cv2.imread(image_file)
    image_copy = image.copy()

    # color clustering based plant object segmentation
    masked_image = color_cluster_seg(image_copy, 'lab', '1', 2, 100)
    cv2.imwrite(join(output_directory, f"{file_name}.mask.png"), masked_image)

    # find external contour and segment image into small ROI based on each plant
    contours_image = comp_external_contour_1(image.copy(), masked_image, output_directory)
    cv2.imwrite(join(output_directory, f"{file_name}.label.png"), contours_image)

    return masked_image


def comp_external_contour_2(orig, thresh):
    # find contours and get the external one
    contours, hier = cv2.findContours(thresh, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)
...
def leaf_traits_computation(orig, labels, output_directory):
    ...
    for index, label in enumerate(np.unique(labels), start=1):
        ...
        # define result path
        cv2.imwrite(join(output_directory, 'leaf_' + str(label) + '.png'), masked)

        # save _skeleton result
        cv2.imwrite(join(output_directory, 'leaf_skeleton_' + str(label) + '.png'), img_as_ubyte(image_skeleton))

        # save color quantization result
        # rgb_colors = color_quantization(image, thresh, save_path, num_clusters)
        (rgb_colors, counts, hex_colors) = color_region(masked, mask, output_directory, num_clusters, False)
    ...
    if n_contours > 0: print('average curvature = {0:.2f}\n'.format(sum(curv_rec) / n_contours))
    else: n_contours = 1.0
...
def color_region(image, mask, save_path, num_clusters, write=True):
    # read the image
    ...
    if write: cv2.imwrite(join(save_path, 'masked.png'), masked_image_ori)
    ...
    # define result path for labeled images
    if write: cv2.imwrite(join(save_path, 'clustered.png'), segmented_image_BRG)
    ...
    for cluster in range(num_clusters):
        ...
        if not cnts:
            ...
            cv2.imwrite(join(save_path, 'result_' + str(cluster) + '.png'), result_BRG)
    ...
    # define result path for labeled images
    plt.savefig(join(save_path, 'pie_color.png'))

    return rgb_colors, counts, hex_colors


def trait_extract(image, output_directory):
    ...
    if isbright(image):
        ...
        (trait_img, area, solidity, max_width, max_height) = comp_external_contour_2(image.copy(), thresh)
        cv2.imwrite(join(output_directory, base_name + '_excontour.png'), trait_img)

        num_clusters = 5
        (rgb_colors, counts, hex_colors) = color_region(image_copy, thresh, output_directory, num_clusters)
        ...
        (avg_curv, label_trait, track_trait, leaf_index_rec, contours_rec, area_rec, curv_rec, solidity_rec, major_axis_rec, minor_axis_rec,
         leaf_color_ratio_rec, leaf_color_value_rec, box_coord_rec) = leaf_traits_computation(image_copy, labels, output_directory)
...
@click.option('-t', '--template', required=False, type=str, default='/opt/smart/marker_template/sticker_template.png')
@click.option('-m', '--multiprocessing', is_flag=True)
def traits(source, output_directory, file_types, template, multiprocessing):
    Path(output_directory).mkdir(parents=True, exist_ok=True)

    if Path(source).is_file():
        segmentation(source, output_directory)
        trait_extract(source, output_directory)
    ...
    #         seg_results = pool.map(trait_extract, images)
    #         traits_results = pool.map(trait_extract, images)
    #         pool.terminate()
...
#     elif Path(source).is_dir():
#         patterns = [ft.lower() for ft in file_types.split(',')]
#         if 'jpg' in patterns: patterns.append('jpeg')
#         if len(patterns) == 0: raise ValueError(f"You must specify file types!")
#
#         patterns = patterns + [pattern.upper() for pattern in patterns]
#         files = sum((sorted(glob(join(source, f"*.{file_type}"))) for file_type in patterns), [])
