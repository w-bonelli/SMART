...
def color_region(image, mask, save_path, base_name, num_clusters, write=True):
    ...
    for cluster in range(num_clusters):
        ...
        if not cnts:
            ...
            cv2.imwrite(join(save_path, base_name + '_result_' + str(cluster) + '.png'), masked_image_rp)
...
def extract_traits(image_file):
    ...
    print("Extracting traits for image: {0}".format(str(image_file_name)))
    ...
    print("Results folder: " + save_path)
    ...
    if isbright(image_file):
        ...
        (avg_curv, label_trait, track_trait, leaf_index_rec, contours_rec, area_rec, curv_rec, solidity_rec, major_axis_rec, minor_axis_rec,
         leaf_color_ratio_rec, leaf_color_value_rec, box_coord_rec) = leaf_traits_computation(orig, labels, save_path, base_name)
...
def extract_traits_2(image, output_directory):
    # gray = cv2.cvtColor(image, cv2.COLOR_BGR2GRAY)
...
if __name__ == '__main__':
    ...
    with closing(Pool(processes=agents)) as pool:
        ...
        result = pool.starmap(extract_traits_2, args)
